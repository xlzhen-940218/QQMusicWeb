 ...
 public class MUtils
 {
   public static M3U8 parseIndex(String url) throws IOException {
     ...
     float seconds = 0.0F; String line;
   ...
   private static float MB = 1024.0F * KB;
   ...
   public static String formatFileSize(long size) {
     if ((float)size >= GB)
       return String.format("%.1f GB", new Object[] { Float.valueOf((float)size / GB) });
     if ((float)size >= MB) {
       float value = (float)size / MB;
       return String.format((value > 100.0F) ? "%.0f MB" : "%.1f MB", new Object[] { Float.valueOf(value) });
     }  if ((float)size >= KB) {
       float value = (float)size / KB;
       return String.format((value > 100.0F) ? "%.0f KB" : "%.1f KB", new Object[] { Float.valueOf(value) });
     }
...
/* Location:              C:\Users\Xiong\Desktop\m3u8.jar!\jaygoo\library\m3u8downloade\\utils\MUtils.class
